/* Created Sun May 12 23:03:10 2019 by frac version 2.5.0 */
...
/* provider */
char *source_info[3] = {"frac","2.5.0","Sun May 12 23:03:10 2019"};
char **sourceinfo () {return source_info;}

/* fiacre and private headers */
#include "2_by_2_2robots.h"
...
int compare_value (struct value *e1, struct value *e2) {
  if ((e1->play_1_vx1) < (e2->play_1_vx1)) return -1;
  if ((e1->play_1_vx1) > (e2->play_1_vx1)) return 1;
  if ((e1->play_1_vy1) < (e2->play_1_vy1)) return -1;
  if ((e1->play_1_vy1) > (e2->play_1_vy1)) return 1;
  if ((e1->play_1_vx2) < (e2->play_1_vx2)) return -1;
  if ((e1->play_1_vx2) > (e2->play_1_vx2)) return 1;
  if ((e1->play_1_vy2) < (e2->play_1_vy2)) return -1;
  if ((e1->play_1_vy2) > (e2->play_1_vy2)) return 1;
  return 0;
}
...
static int sprint_value (int sz, char *buff, key s, const struct pmap *pmap) {
  ...
  PUTS(pmap->r.l);
  PUTN(10,pmap->n,"play_1_vx1"); PUTS(pmap->r.b); PUTV("%hhd",v->play_1_vx1);
  PUTS(pmap->r.s);
  PUTN(10,pmap->n,"play_1_vy1"); PUTS(pmap->r.b); PUTV("%hhd",v->play_1_vy1);
  PUTS(pmap->r.s);
  PUTN(10,pmap->n,"play_1_vx2"); PUTS(pmap->r.b); PUTV("%hhd",v->play_1_vx2);
  PUTS(pmap->r.s);
  PUTN(10,pmap->n,"play_1_vy2"); PUTS(pmap->r.b); PUTV("%hhd",v->play_1_vy2);
  PUTS(pmap->r.r);
...
size_t pack_value (struct value* v, char* data) {
  const char* orig = data;
  data += pack_char(v->play_1_vx1, data);
  data += pack_char(v->play_1_vy1, data);
  data += pack_char(v->play_1_vx2, data);
  data += pack_char(v->play_1_vy2, data);
  return data - orig;
}

size_t unpack_value (struct value* v, char* data) {
  const char* orig = data;
  data += unpack_char(&(v->play_1_vx1), data);
  data += unpack_char(&(v->play_1_vy1), data);
  data += unpack_char(&(v->play_1_vx2), data);
  data += unpack_char(&(v->play_1_vy2), data);
  return data - orig;
}
...
char *transtable[8] = {"play_1_t0","play_1_t1","play_1_t2","play_1_t3","play_1_t4","play_1_t5","play_1_t6","play_1_t7"};
...
bool pre_0 (key s) {
  /* play_1_t0 */
  struct value *v = lookup(s);
  if (!((play_1_va.at[v->play_1_vy1].at[ISUB(v->play_1_vx1,1,0,3)] == 0))) return 0;
  if ((((v->play_1_vx1 - 1) == v->play_1_vx2) && (v->play_1_vy1 == v->play_1_vy2))) return 0;
  return 1;
 error: return 0;
}
...
bool pre_1 (key s) {
  /* play_1_t1 */
  struct value *v = lookup(s);
  if (!((play_1_va.at[v->play_1_vy1].at[IADD(v->play_1_vx1,1,0,3)] == 0))) return 0;
  if ((((v->play_1_vx1 + 1) == v->play_1_vx2) && (v->play_1_vy1 == v->play_1_vy2))) return 0;
  return 1;
 error: return 0;
}
...
bool pre_2 (key s) {
  /* play_1_t2 */
  struct value *v = lookup(s);
  if (!((play_1_va.at[ISUB(v->play_1_vy1,1,0,3)].at[v->play_1_vx1] == 0))) return 0;
  if (((v->play_1_vx1 == v->play_1_vx2) && ((v->play_1_vy1 - 1) == v->play_1_vy2))) return 0;
  return 1;
 error: return 0;
}
...
bool pre_3 (key s) {
  /* play_1_t3 */
  struct value *v = lookup(s);
  if (!((play_1_va.at[IADD(v->play_1_vy1,1,0,3)].at[v->play_1_vx1] == 0))) return 0;
  if (((v->play_1_vx1 == v->play_1_vx2) && ((v->play_1_vy1 + 1) == v->play_1_vy2))) return 0;
  return 1;
 error: return 0;
}
...
bool pre_4 (key s) {
  /* play_1_t4 */
  struct value *v = lookup(s);
  if (!((play_1_va.at[v->play_1_vy2].at[ISUB(v->play_1_vx2,1,0,3)] == 0))) return 0;
  if ((((v->play_1_vx2 - 1) == v->play_1_vx1) && (v->play_1_vy2 == v->play_1_vy1))) return 0;
  return 1;
 error: return 0;
}
...
bool pre_5 (key s) {
  /* play_1_t5 */
  struct value *v = lookup(s);
  if (!((play_1_va.at[v->play_1_vy2].at[IADD(v->play_1_vx2,1,0,3)] == 0))) return 0;
  if ((((v->play_1_vx2 + 1) == v->play_1_vx1) && (v->play_1_vy2 == v->play_1_vy1))) return 0;
  return 1;
 error: return 0;
}
...
bool pre_6 (key s) {
  /* play_1_t6 */
  struct value *v = lookup(s);
  if (!((play_1_va.at[ISUB(v->play_1_vy2,1,0,3)].at[v->play_1_vx2] == 0))) return 0;
  if (((v->play_1_vx2 == v->play_1_vx1) && ((v->play_1_vy2 - 1) == v->play_1_vy1))) return 0;
  return 1;
 error: return 0;
}
...
bool pre_7 (key s) {
  /* play_1_t7 */
  struct value *v = lookup(s);
  if (!((play_1_va.at[IADD(v->play_1_vy2,1,0,3)].at[v->play_1_vx2] == 0))) return 0;
  if (((v->play_1_vx2 == v->play_1_vx1) && ((v->play_1_vy2 + 1) == v->play_1_vy1))) return 0;
  return 1;
 error: return 0;
}
...
key act_0 (key s) {
  ...
  new.play_1_vx1=ISUB(new.play_1_vx1,1,0,3); 
  return store(&new);
 error: return s;
}
...
key act_1 (key s) {
  /* play_1_t1 */
  struct value new = *(struct value*)lookup(s);
  new.play_1_vx1=IADD(new.play_1_vx1,1,0,3); 
  return store(&new);
 error: return s;
}
...
key act_2 (key s) {
  /* play_1_t2 */
  struct value new = *(struct value*)lookup(s);
  new.play_1_vy1=ISUB(new.play_1_vy1,1,0,3); 
  return store(&new);
 error: return s;
}
...
key act_3 (key s) {
  /* play_1_t3 */
  struct value new = *(struct value*)lookup(s);
  new.play_1_vy1=IADD(new.play_1_vy1,1,0,3); 
  return store(&new);
 error: return s;
}
...
key act_4 (key s) {
  /* play_1_t4 */
  struct value new = *(struct value*)lookup(s);
  new.play_1_vx2=ISUB(new.play_1_vx2,1,0,3); 
  return store(&new);
 error: return s;
}
...
key act_5 (key s) {
  /* play_1_t5 */
  struct value new = *(struct value*)lookup(s);
  new.play_1_vx2=IADD(new.play_1_vx2,1,0,3); 
  return store(&new);
 error: return s;
}
...
key act_6 (key s) {
  /* play_1_t6 */
  struct value new = *(struct value*)lookup(s);
  new.play_1_vy2=ISUB(new.play_1_vy2,1,0,3); 
  return store(&new);
 error: return s;
}
...
key act_7 (key s) {
  /* play_1_t7 */
  struct value new = *(struct value*)lookup(s);
  new.play_1_vy2=IADD(new.play_1_vy2,1,0,3); 
  return store(&new);
 error: return s;
}
...
char *obsnames[4] = {"play_1_vx1","play_1_vy1","play_1_vx2","play_1_vy2"};

char **obs_names (int *count) {*count = 4; return obsnames;}
...
struct value *v = lookup(s);
ovalues[0]=(int)(v->play_1_vx1);
ovalues[1]=(int)(v->play_1_vy1);
ovalues[2]=(int)(v->play_1_vx2);
ovalues[3]=(int)(v->play_1_vy2);
return ovalues;
}
